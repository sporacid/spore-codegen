...
namespace spore::codegen
{
    ...
    struct ast_parser_cppast : ast_parser
    {
        ...
        explicit ast_parser_cppast(const codegen_options& options)
            : cppast_parser {
                  type_safe::ref(cppast_index),
                  type_safe::ref(cppast_logger),
              }
        {
            cppast_config.write_preprocessed(false);
            cppast_config.set_flags(cppast::cpp_standard::cpp_latest);
        ...
        {
            cppast::parse_files(cppast_parser, paths, cppast_config);

            if (cppast_parser.error())
            {
                cppast_parser.reset_error();
                SPDLOG_ERROR("unable to parse cpp files");
                return false;
            }
            ...
            try
            {
                std::transform(cppast_parser.files().begin(), cppast_parser.files().end(), std::back_inserter(files),
                    [](const cppast::cpp_file& cppast_file) { return details::parse_file(cppast_file); });
            }
            catch (const cppast::libclang_error& err)
            {
                std::string cause = std::strlen(err.what()) > 0 ? fmt::format(" error={}", err.what()) : "";
                SPDLOG_ERROR("failed to parse cpp files, {}", cause);
                return false;
            }
