...
{
    template <typename ast_t, typename void_t = void>
    ...
    struct codegen_impl
    {
        ...
        codegen_impl(parser_t&& parser, converter_t&& converter)
            // clang-format off
            requires std::derived_from<std::decay_t<parser_t>, codegen_parser<ast_t>>
                 and std::derived_from<std::decay_t<converter_t>, codegen_converter<ast_t>>
        // clang-format on
        {
