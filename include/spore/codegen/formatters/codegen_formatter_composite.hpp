...
namespace spore::codegen
{
    struct codegen_formatter_composite final : codegen_formatter
    {
        ...
        explicit codegen_formatter_composite(std::unique_ptr<formatters_t>&&... in_formatters)
        {
            formatters.reserve(sizeof...(formatters_t));
            (formatters.emplace_back(std::move(in_formatters)), ...);
        }
