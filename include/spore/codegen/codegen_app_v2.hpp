...
namespace spore::codegen
{
    ...
    struct codegen_app_v2
    {
        struct template_data
        {
            std::string path;
            std::string output_suffix;
            codegen_cache::status status;
        };
        ...
        struct step_data
        {
            std::vector<template_data> templates;
        ...
        explicit codegen_app_v2(codegen_options in_options)
            : options(std::move(in_options))
        {
            ...
            //            {
            //                config_path = std::filesystem::absolute(config_path).make_preferred();
            //            }
            //
            ...
            //            {
            //                cache_path = std::filesystem::absolute(output_path / cache_path).make_preferred();
            //            }
            //
            //            options.output = output_path.make_preferred().string();
            //            options.output = output_path.make_preferred().string();
            //            options.cache = cache_path.make_preferred().string();
            ...
            if (cache.check_and_update(options.config) == codegen_cache::status::dirty)
            {
        ...
        inline void run_stage(const codegen_config_stage& stage)
        {
            ...
            const auto predicate = [](const step_data& step) {
                const auto predicate = [](const template_data& template_) {
                    return template_.status != codegen_cache::status::up_to_date;
                };
            ...
            {
                const file_data& file = data.files.at(index);

                if (has_dirty_templates || file.status != codegen_cache::status::up_to_date)
                {
            ...
            {
                const step_data& step = data.steps.at(index_step);
        ...
        //            find_dirty_files(stage, files, templates);
        //
        //            if (files.empty())
        //            {
        //                SPDLOG_INFO("skipping stage, all files are up-to-date, stage={}", stage.name);
        ...
        //            parse_files(stage, files, ast_files);
        //
        //            for (std::size_t index_step = 0; index_step < stage.steps.size(); ++index_step)
        //            {
        ...
        //                }
        //
        //                for (std::size_t index_file = 0; index_file < files.size(); ++index_file)
        //                {
        //                    const std::string& file = files.at(index_file);
        //                    const ast_file& ast_file = ast_files.at(index_file);
        //
        //                    std::vector<std::string> outputs;
        //
        //                    if (condition && condition->matches_condition(ast_file))
        //                    {
        //                        render_file(stage, step, file, ast_file, step_templates, outputs);
        //                    }
        //                }
        //            }
        //        }

        inline void render_file(const step_data& step, const file_data& file, const ast_file& ast)
        {
            std::vector<std::string> outputs;
            for (const template_data& template_ : step.templates)
            {
            ...
            {
                const template_data& template_ = step.templates.at(index);
                const std::string& output = outputs.at(index);
        ...
        //        inline void render_file(const codegen_config_stage& stage, const codegen_config_step& step, const std::string& file, const ast_file& ast_file, const std::vector<std::string>& templates, std::vector<std::string>& outputs2)
        //        {
        //            std::vector<std::string> outputs;
        //            for (const std::string& template_ : step.templates)
        //            {
        //                std::filesystem::path output_ext = std::filesystem::path(template_).stem();
        //                std::filesystem::path output_filename = fmt::format("{}.{}", std::filesystem::path(file).stem().string(), output_ext.string());
        ...
        //                std::filesystem::path output_file = std::filesystem::absolute(output_directory / output_filename).string();
        //                outputs.emplace_back(output_file.string());
        //            }
        //
        //            nlohmann::json json_data;
        //            if (!converter->convert_file(ast_file, json_data))
        //            {
        //                throw codegen_error(codegen_error_code::rendering, "failed to convert input data to json, file={}", file);
        //            }
        //
        //            nlohmann::json json_context;
        //            json_context["outputs"] = outputs;
        //            json_context["user_data"] = user_data;
        //            json_data["$"] = json_context;
        //
        //            for (std::size_t index = 0; index < templates.size(); ++index)
        //            {
        //                const std::string& template_ = templates.at(index);
        //                const std::string& output = outputs.at(index);
        //
        //                SPDLOG_DEBUG("rendering output, file={}", output);
        //
        ...
        //                {
        //                    throw codegen_error(codegen_error_code::rendering, "failed to render input, file={} template={}", file, template_);
        //                }
        //
        //                SPDLOG_DEBUG("writing output, file={}", output);
        //
        ...
        inline void parse_files(const codegen_config_stage& stage, const std::vector<std::string>& files, std::vector<ast_file>& ast_files)
        {
        ...
        //        inline void parse_files_v2(const codegen_config_stage& stage, const stage_data& data)
        //        {
        //            std::vector<ast_file> ast_files;
        //            ast_files.reserve(data.files.size());
        //
        //            SPDLOG_INFO("parsing stage files, stage={} files={} count={}", stage.name, stage.files, files.size());
        //
        //            const auto then = std::chrono::steady_clock::now();
        ...
        inline void make_stage_data(const codegen_config_stage& stage, stage_data& data)
        {
            for (const codegen_config_step& step : stage.steps)
            {
                ...
                const auto transform_template = [&](std::string& template_) {
                    codegen_cache::status status = cache.check_and_update(template_);
                ...
                const auto transform_file = [&](std::filesystem::path& file) {
                    ...
                    codegen_cache::status status = cache.check_and_update(file_string);
                    return file_data {
                ...
                step_data& step_data = data.steps.emplace_back();
        ...
        //        inline void find_dirty_files(const codegen_config_stage& stage, std::vector<std::string>& files, std::vector<std::vector<std::string>>& templates)
        //        {
        //            for (const codegen_config_step& step : stage.steps)
        //            {
        //                std::vector<std::filesystem::path> step_files = glob::rglob(stage.files);
        ...
        //                resolve_templates(step_templates);
        //
        //                for (auto it_template = step_templates.begin(); it_template != step_templates.end();)
        //                {
        //                    if (cache.check_and_update(*it_template) == codegen_cache::status::up_to_date)
        //                    {
        //                        it_template = step_templates.erase(it_template);
        //                    }
        //                    else
        //                    {
        //                        ++it_template;
        ...
        //                }
        //
        //                for (std::filesystem::path& step_file : step_files)
        //                {
        //                    std::string step_file_string = step_file.string();
        //                    if (cache.check_and_update(step_file_string) == codegen_cache::status::dirty || !step_templates.empty())
        //                    {
        //                        files.emplace_back(std::move(step_file_string));
        //                    }
        //                }
        //            }
        //
